...
const Roulette: MsgCommand = {
  ...
  exe: async ({ msg, args, username, authorID, pl, client }) => {
    ...
    if (roulette.includes(authorID)) {
      await msg.reply(
        "**You already requested a Roulette table! Finish your match before starting another.**"
      );
      return;
    }
    let n = Number(args[0]);
    ...
    cl.on("end", async (coll) => {
      ...
      ids.push(readyID);
      let cl = msg.channel.createMessageComponentCollector({
        filter: filters.roulette(ids, joined, uBets),
        idle: 6e4,
      ...
      cl.on("collect", async (btnInt) => {
        if (btnInt.customId === readyID) {
          await btnInt.deferUpdate();
          readys++;
          if (readys === joinedName.length) cl.stop();
        ...
        ebd.setFooter({
          text: `Players ready: ${readys}/${joined.length}`,
          iconURL: EmbedAssets.ProPic,
        });
        await botMsg.edit({
          embeds: [ebd],
